...
from .losses import (
    estimate_diag_huber_delta_from_trainset,
    estimate_loss_hparams_from_trainset,
    evaluate_masked_tensor_metrics,
    weighted_masked_huber_loss,
)
from .metrics import compute_original_tensor_metrics
...
def build_checkpoint_config(config):
    return {
        ...
        "loss_ws": config["loss_ws"],
        "loss_wq": config.get("loss_wq"),
        "huber_delta": config.get("huber_delta"),
        "huber_delta_diag": config.get("huber_delta_diag"),
...
def run_training(config):
    ...
    estimated_delta_diag = estimate_diag_huber_delta_from_trainset(train_dataloader.dataset)
    checkpoint_config["loss_wq"] = estimated_wq
    checkpoint_config["huber_delta"] = estimated_delta
    checkpoint_config["huber_delta_diag"] = estimated_delta_diag

    print(f"Auto-estimated loss_wq: {checkpoint_config['loss_wq']}")
    print(f"Auto-estimated huber_delta: {checkpoint_config['huber_delta']:.6f}")
    print(f"Auto-estimated huber_delta_diag [xx, yy, zz]: {checkpoint_config['huber_delta_diag']}")
    ...
    print("Best model selection metric: valid_mae")
    ...
    for epoch in range(config["epochs"]):
        ...
        for batch in train_bar:
            ...
            with autocast():
                output = net(batch)
                loss = weighted_masked_huber_loss(
                    output,
                    batch.energy,
                    batch.energy_mask,
                    ws=checkpoint_config["loss_ws"],
                    wq=checkpoint_config["loss_wq"],
                    delta=checkpoint_config["huber_delta_diag"],
                )
        ...
        with torch.no_grad():
            for batch in valid_bar:
                ...
                with autocast():
                    ...
                    if target_transform is not None and hasattr(batch, "energy_raw"):
                        output_raw = target_transform.inverse_tensor(output)
                        batch_mae_raw, batch_rmse_raw, batch_max_abs_raw = evaluate_masked_tensor_metrics(
                            output_raw, batch.energy_raw, batch.energy_mask
                        )
                    else:
                        batch_mae_raw, batch_rmse_raw, batch_max_abs_raw = (
                            batch_mae,
                            batch_rmse,
                            batch_max_abs,
                        )
    ...
    print(f"Training finished. Best valid_mae = {best_valid_mae:.6e}")
    ...
    if os.path.exists(config["best_model_path"]):
        ...
        with torch.no_grad():
            for batch in test_dataloader:
                ...
                if target_transform is not None and hasattr(batch, "energy_raw"):
                    output_for_export = target_transform.inverse_tensor(output)
                    target_for_export = batch.energy_raw
                    batch_mae_raw, batch_rmse_raw, batch_max_abs_raw = evaluate_masked_tensor_metrics(
                        output_for_export, target_for_export, batch.energy_mask
                    )
                else:
                    output_for_export = output
                    target_for_export = batch.energy
                    batch_mae_raw, batch_rmse_raw, batch_max_abs_raw = (
                        batch_mae,
                        batch_rmse,
                        batch_max_abs,
